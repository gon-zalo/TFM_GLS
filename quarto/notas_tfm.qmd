...
plot_similarity <- function(data) {
  ggplot(data, aes(x = type, y = mean_similarity, fill = type)) +
    ...
    theme(
      ...
      axis.text.x = element_text(angle = -30, hjust = 0, vjust = 1, size = 8),
...
ggsave("plots/fig-clean.pdf", plot = plot4, width = 6, height = 5, units = "in")
```
...
plot8 <- ggplot(baseline_data, aes(x = mean_similarity, fill = type)) +
  geom_histogram(binwidth = 0.00653, alpha = 0.7, position = "identity", color = 'black', size = 0.01) +
  labs(x = "Mean similarity", y = "Count", fill = "Type") +
  facet_grid(type ~ language ~ model) +
  geom_vline(data = overall_mean_data, aes(xintercept = overall_mean, linetype = "Threshold"), color = "black", linewidth = 0.5) +
  scale_linetype_manual(values = c("Threshold" = "dotted")) + # name = ""
  theme_classic() +
  theme(
    ...
    legend.key.size = unit(0.4, "cm"),  # size of the boxes
    legend.spacing.x = unit(0.2, "cm"),  # horizontal spacing between keys
    plot.margin = margin(t = 0, r = 25, b = 0, l = 2),
...
# ggsave("plots/fig-baseline-faceted.pdf", plot = plot8, width = 6, height = 9, units = "in")
```
...
prf_ft <- read.csv("results/pol/pol_fasttext_aspect_results.csv")
prf_w2v <- read.csv("results/pol/pol_word2vec_aspect_results.csv")

prf_ft <- prf_ft %>%  
  mutate(model = "FastText")

prf_w2v <- prf_w2v %>%  
  mutate(model = "Word2Vec")

aspect_df <- bind_rows(prf_ft, prf_w2v)

aspect_df <- aspect_df %>%  
  mutate(model = as.factor(model), pair_type = as.factor(pair_type), aspect = as.factor(aspect), pair_aspect = as.factor(pair_aspect))

aspect_df <- aspect_df %>% 
  filter(pair_type != "suppletion")

aspect_plot <- ggplot(aspect_df, aes(y = similarity, fill = model, x = model)) +
  geom_dotplot(
      binaxis = "y", stackdir = "center", 
      dotsize = 5, binwidth = 0.005, 
  ...
  guides(fill = "none") +
  facet_grid(pair_type ~ model, scales = "free_x", space = "free_x")

aspect_plot

ggsave("plots/aspect_plot.pdf", plot = aspect_plot, width = 6, height = 9, units = "in")

```

```{r}
summary_stats <- aspect_df %>%
  group_by(model, pair_type) %>%
  summarise(
    count = n(),
    mean_similarity = mean(similarity, na.rm = TRUE),
    sd_var = sd(similarity, na.rm = TRUE),
    median_var = median(similarity, na.rm = TRUE),
    min_var = min(similarity, na.rm = TRUE),
    max_var = max(similarity, na.rm = TRUE)
