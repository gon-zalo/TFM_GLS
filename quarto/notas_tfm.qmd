...
# write.csv(all_df_plot, "results/clean_data.csv", row.names=FALSE)
```
...
clean_data$model <- factor(clean_data$model, levels = c("FastText", "Word2Vec", "Multilingual BERT"))

plot1 <- plot_similarity(clean_data)
...
subset_data$model <- factor(subset_data$model, levels = c("FastText", "Word2Vec", "Multilingual BERT"))

plot2 <- plot_similarity(subset_data)
...
shuffled_data <- read.csv("results/shuffled_data.csv")

shuffled_data <- prepare_dataset(shuffled_data)

shuffled_data$model <- factor(shuffled_data$model, levels = c("FastText", "Word2Vec", "Multilingual BERT"))

plot3 <- plot_similarity(shuffled_data)
...
clean_data_v2 <- read.csv("results/clean_data_v2.csv")

clean_data_v2 <- prepare_dataset(clean_data_v2)

clean_data_v2$model <- factor(clean_data_v2$model, levels = c("FastText", "Word2Vec", "BERT"))

plot4 <- plot_similarity(clean_data_v2)
...
random_baseline <- read.csv("results/random_baseline.csv")

random_baseline <- prepare_dataset(random_baseline)

random_baseline$model <- factor(random_baseline$model, levels = c("FastText", "Word2Vec", "BERT"))

plot5 <- plot_similarity(random_baseline)
...
### Subset results using RoBERTa and PolBERT
...
# write.csv(all_df_plot, "results/subset_data_v2.csv", row.names=FALSE)
```
...
subset_data_v2 <- read.csv("results/subset_data_v2.csv")

subset_data_v2 <- prepare_dataset(subset_data_v2)

subset_data_v2$model <- factor(subset_data_v2$model, levels = c("FastText", "Word2Vec", "BERT"))

plot6 <- plot_similarity(subset_data_v2)

plot6

# ggsave("plots/fig-subset-v2.pdf", plot = plot1, width = 22, height = 14, units = "in")
```
...
big_df <- bind_rows(clean_data, shuffled_data, subset_data, random_baseline, clean_data_v2, subset_data_v2)

big_df <- prepare_dataset(big_df)

# write.csv(big_df, "results/random_baseline_v2.csv", row.names=FALSE)
```

<!-- ```{r}

fasttext_df <- big_df[big_df$model == "FastText", ]
word2vec_df <- big_df[big_df$model == "Word2Vec", ]

missing_in_word2vec <- fasttext_df[!(fasttext_df$some_id_column %in% word2vec_df$some_id_column), ]


anti_join(fasttext_df, word2vec_df, by = c("category"))
...
# missing_in_word2vec <- fasttext_df[!(fasttext_df$category %in% word2vec_df$category), ]

anti_join(fasttext_df, word2vec_df, by = "category")


fasttext_df <- unique(fasttext_df)
word2vec_df <- unique(word2vec_df)
library(dplyr)

# Filter and keep only unique rows
fast_unique <- fasttext_df %>% distinct(category)
word2vec_unique <- word2vec_df %>% distinct(category)

# Now compare
missing_in_word2vec <- anti_join(fast_unique, word2vec_unique, 
                                 by = "category")

nrow(missing_in_word2vec)


```

```{r}
fast_pairs <- fasttext_df %>% distinct(category)
word2vec_pairs <- word2vec_df %>% distinct(category)

missing_pairs <- anti_join(fast_pairs, word2vec_pairs, by = "category")
nrow(missing_pairs)
