...
![Mean similarity between pivot-form in derivation and inflection by model and language](C:/PythonCode/TFM_GLS/py/plots/fig-initial.svg){#fig-initial}
...
The first analysis revealed some errors in both datasets. In the derivations dataset the label U (unspecified or unknown) presented some issues. The goal is to eliminate all instances of unknown categories, to get rid of this noise and have cleaner results and means.
...
In order to clean the Spanish derivations dataset we assign a new category according to the affix. We change all the affixes that end in *-ero*, *-ez*, *-ismo*,*-í* and *-illa* to N. We also assign V to those that contain the affixes *-ar* and *-ear*. As a result we obtain 6 rows that can be eliminated from the final dataset because of all the mistakes they contain.
...
Regarding the data labeled as U:X, we can do much better than in Spanish because there are many rows (163) that contain verbs ending in *-ć* in both the pivot and the derivation column but are incorrectly labeled as U:V, for example *kręcić skręcic U:V s-* or *paść przepaść U:V prze-*. We can easily change the label to V:V. There are also 27 rows that contain verbs ending in *-c* in both the pivot and the derivation cells, which can be fixed just like previously done on the other group of verbs.
...
annotate_dataset <- function(data, model, language, type) {
  data %>%
    mutate(model = model, language = language, type = type)
}
...
spa_ft_inf <- annotate_dataset(spa_ft_inf, 'FastText', 'Spanish', 'Inflection')
pol_ft_inf <- annotate_dataset(pol_ft_inf, 'FastText', 'Polish', 'Inflection')
spa_ft_der <- annotate_dataset(spa_ft_der, 'FastText', 'Spanish', 'Derivation')
pol_ft_der <- annotate_dataset(pol_ft_der, 'FastText', 'Polish', 'Derivation')

spa_w2v_inf <- annotate_dataset(spa_w2v_inf, 'Word2Vec', 'Spanish', 'Inflection')
pol_w2v_inf <- annotate_dataset(pol_w2v_inf, 'Word2Vec', 'Polish', 'Inflection')
spa_w2v_der <- annotate_dataset(spa_w2v_der, 'Word2Vec', 'Spanish', 'Derivation')
pol_w2v_der <- annotate_dataset(pol_w2v_der, 'Word2Vec', 'Polish', 'Derivation')

spa_bert_inf <- annotate_dataset(spa_bert_inf, 'Multilingual BERT', 'Spanish', 'Inflection')
pol_bert_inf <- annotate_dataset(pol_bert_inf, 'Multilingual BERT', 'Polish', 'Inflection')
spa_bert_der <- annotate_dataset(spa_bert_der, 'Multilingual BERT', 'Spanish', 'Derivation')
pol_bert_der <- annotate_dataset(pol_bert_der, 'Multilingual BERT', 'Polish', 'Derivation')
...
all_df$model <- factor(all_df$model, levels = c("FastText", "Word2Vec", "Multilingual BERT"))
...
plot1 <- plot_similarity(all_df_plot)

plot1

# ggsave("C:/PythonCode/TFM_GLS/py/plots/fig-initial-clean.pdf", plot = plot1, width = 22, height = 14, units = "in")
```
...
plot2 <- plot_similarity(all_df_plot)

plot2

# ggsave("C:/PythonCode/TFM_GLS/py/plots/fig-subset-plot.pdf", plot = plot1, width = 22, height = 14, units = "in")
```
...
import pandas as pd
df = pd.read_csv("C:/PythonCode/TFM_GLS/py/datasets/spa/spa_derivations_shuffled.txt", sep="\t", header=None, names=["pivot", "derivation", "category", "affix"])
...
# FASTTEXT
# INFLECTION
spa_ft_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_fasttext_inflection_shuffled_results.csv")
pol_ft_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_fasttext_inflection_shuffled_results.csv")
# DERIVATION
spa_ft_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_fasttext_derivation_shuffled_results.csv")
pol_ft_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_fasttext_derivation_shuffled_results.csv")

# WORD2VEC
# INFLECTION
spa_w2v_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_word2vec_inflection_shuffled_results.csv")
pol_w2v_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_word2vec_inflection_shuffled_results.csv")
# DERIVATION
spa_w2v_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_word2vec_derivation_shuffled_results.csv")
pol_w2v_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_word2vec_derivation_shuffled_results.csv")
...
plot3 <- plot_similarity(all_df_plot)

plot3

# ggsave("C:/PythonCode/TFM_GLS/py/plots/fig-random-baseline.pdf", plot = plot1, width = 22, height = 14, units = "in")
```

One thing is apparent right away, the Multilingual BERT results do not make any sense. The problem might be that this particular BERT model is trained on many languages and they are interfering with the task. To test this I tested two other BERT based models, one for Polish (PolBERT) and one for Spanish (RoBERTa), the results should be better since these models have only access to a single language.
...
# BERT
# INFLECTION
spa_bert_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_roberta_inflection_results.csv")
pol_bert_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_polbert_inflection_results.csv")
# DERIVATION
spa_bert_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_roberta_derivation_results.csv")
pol_bert_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_polbert_derivation_results.csv")
```
...
spa_bert_inf <- annotate_dataset(spa_bert_inf, 'BERT', 'Spanish', 'Inflection')
pol_bert_inf <- annotate_dataset(pol_bert_inf, 'BERT', 'Polish', 'Inflection')
spa_bert_der <- annotate_dataset(spa_bert_der, 'BERT', 'Spanish', 'Derivation')
pol_bert_der <- annotate_dataset(pol_bert_der, 'BERT', 'Polish', 'Derivation')
...
all_df$model <- factor(all_df$model, levels = c("FastText", "Word2Vec", "BERT"))
...
plot5 <- plot_similarity(all_df_plot)

plot5

# ggsave("C:/PythonCode/TFM_GLS/py/plots/fig-clean-bert2.pdf", plot = plot1, width = 22, height = 14, units = "in")
```
...
# BERT
# INFLECTION
spa_bert_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_roberta_inflection_shuffled_results.csv")
pol_bert_inf <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_polbert_inflection_shuffled_results.csv")
# DERIVATION
spa_bert_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/spa/spa_roberta_derivation_shuffled_results.csv")
pol_bert_der <- read.csv("C:/PythonCode/TFM_GLS/py/results/pol/pol_polbert_derivation_shuffled_results.csv")
...
plot4 <- plot_similarity(all_df_plot)

plot4

# ggsave("C:/PythonCode/TFM_GLS/py/plots/fig-shuffled-bert2.pdf", plot = plot1, width = 22, height = 14, units = "in")
```
