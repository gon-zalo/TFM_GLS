...
In order to draw a boundary between both morphological processes authors propose many different criteria. 
...
Another criterion, a relevant one regarding this thesis, is that in inflection the part of speech or category of the base  does not change. Derivational processes on the other hand typically change the category, although not always. There exist both category-preserving and category-changing derivation, since a change in lexical meaning does not mean a change of category \parencite{,booij2006InflectionDerivation,haspelmath2013UnderstandingMorphology,stump2017Inflection} like it was shown in the examples before. There are also some cases in which inflection is involved in a change of category, like in gerunds (infinitives in the case of Romance languages) or participles, which are part of the verbal paradigm but can also function as nouns or adjectives \parencite{booij2012GrammarWordsIntroduction,stekauer2015DelimitationDerivationInflection,cappellaro2022InflexionDerivationCompounding,haspelmath2024InflectionDerivationTraditional}. \textcite{stump2005WordFormationInflectionalMorphology} points out that, in many languages, participles never appear without the declension morphology of adjectives, e.g. Polish. % extend this argument? carmen doesnt like it
The most relevant criterion in this thesis is the semantic regularity criterion. It states that derivation expresses a new meaning but inflection does not change the lexical meaning \parencite{haspelmath2013UnderstandingMorphology} or, as put by \textcite{stump2017Inflection}, inflectional processes are semantically more regular than derivational processes. Lexemes formed by derivation are often hard to predict semantically, e.g. words ending in \ita{-ize} in English such as \ita{dollarize}, \ita{hospitalize} or \ita{vaporize} \parencite{stump2005WordFormationInflectionalMorphology,stump2017Inflection, haspelmath2024InflectionDerivationTraditional}. Sometimes, it is true that a related noun and adjective like \ita{czystość} `cleanliness' and \ita{czysty} `clean' are also clearly related in meaning, but this relation is not like that of a verb and a conjugated form, like \ita{comer} `eat' and \ita{comiste} `you ate' \parencite{aronoff2011WhatMorphology}. In fact, \textcite{haspelmath2013UnderstandingMorphology} argue examples like the first one fall in between inflection and derivation.
